...
PuntoVentaRouters.get("/", async (req, res) => {
  try {
    var data = await buscarElasticByType("company");
...
PuntoVentaRouters.post("/", async (req, res) => {
  try {
    var recinto = {};
    const data = req.body;
    const response = await crearElasticByType(data, "company");
    //recinto = response.body;

    return res
      .status(201)
      .json({ message: "company created.", recinto, data });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
});
...
PuntoVentaRouters.patch("/:id", async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "company updated.", response, data });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
});
